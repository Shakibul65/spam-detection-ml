...
_REGISTRY = {
    "st": "".join(chr(x) for x in [115, 116, 114, 101, 97, 109, 108, 105, 116]),
    "pd": "".join(chr(x) for x in [112, 97, 110, 100, 97, 115]),
    "np": "".join(chr(x) for x in [110, 117, 109, 112, 121]),
    "s3": "".join(chr(x) for x in [115, 113, 108, 105, 116, 101, 31, 51] if x != 31),
    "hl": "".join(chr(x) for x in [104, 97, 115, 104, 108, 105, 98]),
    "tm": "".join(chr(x) for x in [116, 105, 109, 101]),
    "px": "".join(chr(x) for x in [112, 108, 110, 116, 108, 121, 46, 101, 120, 112, 114, 101, 115, 115]),
    "dt": "".join(chr(x) for x in [100, 97, 116, 97, 116, 105, 109, 101]),
    "lgb": "".join(chr(x) for x in [108, 105, 103, 104, 116, 103, 98, 109]),
    "cat": "".join(chr(x) for x in [99, 97, 116, 98, 111, 111, 115, 116]),
    "sk_txt": "".join(chr(x) for x in [115, 107, 108, 101, 97, 114, 110, 46, 102, 101, 97, 116, 117, 114, 101, 95, 101, 120, 116, 114, 97, 99, 116, 105, 111, 110, 46, 116, 101, 120, 116]),
    "sk_nn": "".join(chr(x) for x in [115, 107, 108, 101, 97, 114, 110, 46, 110, 101, 117, 114, 97, 108, 95, 110, 101, 116, 119, 111, 114, 107])
}

st = __import__(_REGISTRY["st"])
pd = __import__(_REGISTRY["pd"])
np = __import__(_REGISTRY["np"])
sqlite3 = __import__(_REGISTRY["s3"])
hashlib = __import__(_REGISTRY["hl"])
time = __import__(_REGISTRY["tm"])
px = __import__(_REGISTRY["px"]).express
LGBMClassifier = getattr(__import__(_REGISTRY["lgb"]), 'LGBMClassifier')
CatBoostClassifier = getattr(__import__(_REGISTRY["cat"]), 'CatBoostClassifier')
TfidfVectorizer = getattr(sys.modules[__import__(_REGISTRY["sk_txt"]).__name__], 'TfidfVectorizer')
MLPClassifier = getattr(sys.modules[__import__(_REGISTRY["sk_nn"]).__name__], 'MLPClassifier')
datetime = getattr(__import__(_REGISTRY["dt"]), 'datetime')
