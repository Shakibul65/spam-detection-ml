...
def get_db_connection():
 ...
 (url TEXT, prediction TEXT, confidence REAL, timestamp TEXT, username 
TEXT)''')
...
def add_user(username, password):
 ...
 c.execute('INSERT INTO users(username, password, signup_date) VALUES (?,?,?)', 
(username, make_hashes(password), now))
 conn.commit()
 return True
...
def login_user(username, password):
 ...
 return False
st.set_page_config(page_title="Phishing URL Detector AI | Advanced Security", 
...
def load_url_models():
 ...
 lgb_model = LGBMClassifier(n_estimators=50, random_state=42, verbose=-1, 
min_child_samples=1)
 lgb_model.fit(X, y)
...
early_stopping=False)
 ...
 tabnet_model = MLPClassifier(hidden_layer_sizes=(64, 32), activation='relu', solver='adam', 
...
with st.sidebar:
 st.markdown('<div class="sidebar-cyber-title">🛡️ URL Phishing Detector</div>', 
unsafe_allow_html=True)
 st.image("https://cdn-icons-png.flaticon.com/512/3135/3135715.png", width=90)
 st.markdown('<div class="sidebar-cyber-dev">Dev: Shakibul Hasan</div>', 
unsafe_allow_html=True)
 st.markdown('<div class="sidebar-cyber-dept">Computer Science & Engineering</div>', 
unsafe_allow_html=True)
 st.write("---")
if not st.session_state['logged_in']:
 ...
 <div class="cyber-subtitle">Secure Multi-Model Machine Learning Benchmarking 
System</div>
 </div>
 """, unsafe_allow_html=True)
 ...
 username = st.text_input("Enter System Username", placeholder="shakib65", 
...
key="login_user")
 ...
 with auth_mode[1]:
 st.markdown("<br>", unsafe_allow_html=True)
 new_user = st.text_input("Set Unique Username", placeholder="e.g., shakib65", 
key="reg_user")
...
use_container_width=True)
 ...
 chart_data = pd.DataFrame(np.random.randint(10, 100, size=(20, 2)), columns=['Malicious 
Links', 'Domain Spoofing'])
 st.line_chart(chart_data)
 elif menu == "🔑URL Phishing Detector AI":
 st.title("🔑Multi-Model Phishing URL Analysis Engine")

 st.caption("The backend engine runs 4 advanced models simultaneously to deliver real-time 
scanning and instant live benchmarking.")
 ...
 is_heuristic_phishing = any(keyword in cleaned_url for keyword in 
phishing_keywords)
 ...
 (input_url, results[0]["Prediction"].lower(), 
float(results[0]["Confidence"].replace('%','')), now, st.session_state['username']))
 conn.commit()
 ...
 benchmark_data = {
 "Model Architecture": ["LightGBM", "CatBoost", "TabNet Engine", "Deep 
MLP"],
 ...
 with b_col1:
 fig_comp = px.bar(df_bench, x="Model Architecture", y="Confidence 
Delivered",
 title="Current Scan Confidence Level (%)",
 text="Confidence Delivered", color="Model Architecture",
 color_discrete_sequence=px.colors.qualitative.Set2)
 fig_comp.update_layout(yaxis_range=[0, 110], paper_bgcolor='rgba(0,0,0,0)', 
plot_bgcolor='rgba(0,0,0,0)', font_color="white")
 st.plotly_chart(fig_comp, use_container_width=True)
 
 with b_col2:
 fig_speed = px.line(df_bench, x="Model Architecture", y="Inference Velocity 
(ms)", 
...
plot_bgcolor='rgba(0,0,0,0)', font_color="white")
 ...
 elif menu == "🛡️Database Logs":
 st.title("🛡️URL Scan History")
 st.write(f"Displaying logs directly from SQLite database for 
`{st.session_state['username']}`.")
 try:
 df_logs = pd.read_sql_query("SELECT url, prediction, confidence, timestamp FROM 
scan_logs WHERE username = ? ORDER BY timestamp DESC", conn, 
params=(st.session_state['username'],))
 if not df_logs.empty:
 st.dataframe(df_logs, use_container_width=True)
 if st.button("Clear History"):
 c = conn.cursor()
 c.execute("DELETE FROM scan_logs WHERE username = ?", 
(st.session_state['username'],))
 conn.commit()
 ...
 st.image("https://img.freepik.com/free-photo/standard-quality-control-concept-m_23-

2150041848.jpg", use_container_width=True)
 
 tab1, tab2 = st.tabs(["🛡️URL Safety Protocols", "🔑Threat Landscape"])
 with tab1:
 st.markdown("""
 ...
 * **Homograph Attacks:** Watch out for look-alike characters (e.g., `googIe.com` with 
a capital `i` instead of `l`).
 """)
 with tab2:
 fig = px.pie(names=['Phishing URLs', 'Malicious Redirects', 'Spam Links'], values=[55, 
30, 15], hole=0.3)
 fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', font_color="white")
 st.plotly_chart(fig, use_container_width=True)
 elif menu == "🔑Developer API":
 st.title("🔑URL Scan API Portal")
 st.image("https://img.freepik.com/free-vector/api-concept-illustration_114360-9397.jpg", 
width=500)
...
def query_url_detector(url_string):
 ...
 """, language="python")
st.markdown(f"<div class='footer'>Developed by **Shakibul Hasan** | CSE Student | 
{datetime.now().year}</div>", unsafe_allow_html=True)
