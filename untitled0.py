...
# ==========================================
# 3. Secure AI Model Architecture (No Caching Conflict)
# ==========================================
if 'init_done' not in st.session_state:
    ...
    tfidf_vectorizer = TfidfVectorizer(analyzer='char', ngram_range=(3, 5), max_features=150)
    X = tfidf_vectorizer.fit_transform(df['url']).toarray()
    y = df['label'].map({'safe': 0, 'phishing': 1}) if df['label'].dtype == 'object' else df['label']
        
    lgb_engine = LGBMClassifier(n_estimators=15, random_state=42, verbose=-1, n_jobs=1)
    lgb_engine.fit(X, y)
    
    cat_engine = CatBoostClassifier(iterations=15, random_state=42, verbose=0, thread_count=1)
    cat_engine.fit(X, y)
    
    mlp_engine = MLPClassifier(hidden_layer_sizes=(64, 32), max_iter=250, random_state=42)
    mlp_engine.fit(X, y)
    
    tabnet_engine = MLPClassifier(hidden_layer_sizes=(128, 64), activation='relu', solver='adam', random_state=42)
    tabnet_engine.fit(X, y)
    
    st.session_state['tfidf_vectorizer'] = tfidf_vectorizer
    st.session_state['models_pool'] = {
        "LightGBM": lgb_engine,
        "CatBoost": cat_engine,
        "TabNet Engine": tabnet_engine,
        "Deep MLP": mlp_engine
    }
    st.session_state['init_done'] = True

tfidf_vectorizer = st.session_state['tfidf_vectorizer']
models_pool = st.session_state['models_pool']
...
# ==========================================
# 5. Core Operational Application Sections
# ==========================================
...
if menu == "🏠 Master Dashboard":
    ...
    if st.button("Start AI Scan 🚀"):
        if input_url:
            with st.spinner('Running advanced URL string & heuristic cross-verification...'):
                ...
                vect = tfidf_vectorizer.transform([input_url.lower()]).toarray()
                
                # সম্পূর্ণ নতুন ডাইরেক্ট স্কোপ ভেরিয়েবল (এররলেস)
                pipeline_final_reports = []
                
                for algo_name, current_model in models_pool.items():
                    pred_code = current_model.predict(vect)[0]
                    prob = current_model.predict_proba(vect)[0]
                    conf = max(prob) * 100
                    res_text = 'PHISHING' if pred_code == 1 else 'SAFE'
                    
                    pipeline_final_reports.append({
                        "Algorithm": algo_name,
                        "Prediction": res_text,
                        "Confidence": conf,
                ...
                c.execute('INSERT INTO scan_logs VALUES (?,?,?,?)', (input_url, pipeline_final_reports[0]["Prediction"].lower(), pipeline_final_reports[0]["Confidence"], now))
                conn.commit()
                
                # 1. কার্ড ম্যাট্রিক্স
                st.write("### 📊 Advanced Hybrid AI Scan Results:")
                cols = st.columns(4)
                for idx, r in enumerate(pipeline_final_reports):
                    with cols[idx]:
                        st.markdown(f"""
                ...
                # 2. ভিজ্যুয়াল গ্রাফিক্স চার্টস সেকশন
                ...
                benchmark_data = {
                    ...
                    "Confidence Delivered": [
                        pipeline_final_reports[0]["Confidence"],
                        pipeline_final_reports[1]["Confidence"],
                        pipeline_final_reports[2]["Confidence"],
                        pipeline_final_reports[3]["Confidence"]
                    ],
                ...
                with b_col1:
                    fig_comp = px.bar(df_bench, x="Model Architecture", y="Confidence Delivered",
                                      title="Current Scan Confidence Level (%)", text="Confidence Delivered", 
                                      color="Model Architecture", color_discrete_sequence=px.colors.qualitative.Set2)
                ...
                with b_col2:
                    fig_speed = px.line(df_bench, x="Model Architecture", y="Inference Velocity (ms)", 
                                        title="Execution Speed / Latency (Lower is Better)", markers=True)
    ...
    st.write("Upload a CSV file containing a 'url' column for high-volume analysis.")
    uploaded_file = st.file_uploader("Choose CSV file", type="csv")
    if uploaded_file:
        df_batch = pd.read_csv(uploaded_file)
        st.write("Preview of URLs:", df_batch.head())
        if st.button("Analyze Batch URLs"):
            st.info("Bulk analysis routine initiated successfully.")
    ...
    try:
        ...
        if not df_logs.empty:
            ...
            st.info("No URL records tracked inside the SQL database yet.")
    ...
    with tab1:
        ...
        * **Homograph String Injections:** Look closely for look-alike characters (e.g., `googIe.com` using a capital `i`).
        """)
    with tab2:
...
def query_url_detector(url_string):
    ...
    return requests.post(api_endpoint, json=payload).json()
    """, language="python")
